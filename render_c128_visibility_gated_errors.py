...
def main():
    ...
    masks={'front':np.asarray(Image.open(BASE/'raw_ovoxel_render/alpha.png').convert('L'))>127,
           'back':np.asarray(Image.open(OUT/'baseline1024_reference/back/mask.png').convert('L'))>127}
    rows=[]
    for view in ('front','back'):
        reference=refs[view];mask=masks[view];assert mask.any()
        gap=24;size=1024;label=140;header=130
        ...
        for index,record in enumerate(records):
            ...
            draw.text((x+12,y+8),f"n={record['n']:02d} | guided {record['n']} / gated {12-record['n']}",font=font,fill='white')
    ...
    for col,view in enumerate(('front','back')):
        ...
        for axis,key,ylabel in zip(axes[:,col],('foreground_psnr_db','foreground_ssim'),('PSNR (dB)','SSIM')):
            ...
            axis.set(xlabel='Endpoint-guided prefix n (remaining 12-n: visibility-gated)',ylabel=ylabel,xticks=ns)
            axis.grid(alpha=.25)
        axes[0,col].set_title('Input view vs canonical input' if view=='front' else 'Back view vs baseline back (not GT)')
    fig.suptitle('C128 visibility-gated flow: fixed-foreground metrics',fontsize=17)
