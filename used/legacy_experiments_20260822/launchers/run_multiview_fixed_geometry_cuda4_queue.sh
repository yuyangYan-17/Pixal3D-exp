#!/usr/bin/env bash
# Unattended CUDA-4 execution for the fixed-geometry multi-view study.
# The full flow and final render run serially; no second process is polled.
set -Eeuo pipefail

repo_dir="/home/nvme04/yyyan/Pixal3D"
python_bin="/home/nvme04/yyyan/miniconda3/envs/pixal3d/bin/python"
queue_dir="$repo_dir/outputs/multiview_fixed_geometry_pbr_gaussian_cuda4_queue"
...
run_stage() {
  ...
  (
    ...
    env -u CUDA_VISIBLE_DEVICES CUDA_DEVICE_ORDER=PCI_BUS_ID HF_HUB_OFFLINE=1 \
      ...
      --cuda-device 4 --flow-batch-size 8 --output-dir "$output_dir" "$@"
...
full_output="$repo_dir/outputs/multiview_fixed_geometry_pbr_gaussian_cuda4_rework_b8"
...
(
  ...
  env -u CUDA_VISIBLE_DEVICES CUDA_DEVICE_ORDER=PCI_BUS_ID HF_HUB_OFFLINE=1 \
    "$python_bin" pixal3d_multiview_fixed_geometry_pbr_gaussian_sr.py \
    --cuda-device 4 --render-only --output-dir "$full_output"
