...
Additional comparison route:

    after global token selection, drop from each tile all source-global tokens that
    lost the global selection for that tile
    -> keep only the surviving local C64 tokens touched by the surviving source-global
       tokens
    -> decode each tile locally from those filtered tile SLAT features
    -> convert decoder object coordinates to local q, apply the inverse camera mapping,
       and convert global q back to global object coordinates
    -> crop triangles and O-Voxels by projected tile ownership
    -> merge decoder metadata from all tiles on a full canonical 1024^3 O-Voxel lattice
    -> render the merged overall result and compare it with the global baseline
...
def _decode_filtered_tile_payloads_and_merge_to_global(
    *,
    pipeline: Any,
    payloads: Sequence[TileLocalDecodePayload],
    selected_global_coords64: torch.Tensor,
    selected_tile_ids: torch.Tensor,
    global_camera: Mapping[str, float],
    template_voxel_shape: Any,
    ...
    if selected_global_coords64.ndim != 2 or selected_global_coords64.shape[1] != 4:
        raise ValueError("selected_global_coords64 must be [N,4]")
    if selected_tile_ids.ndim != 1 or selected_tile_ids.shape[0] != selected_global_coords64.shape[0]:
        raise ValueError("selected_tile_ids must be [N] and align with selected_global_coords64")

    selected_coords_cpu = selected_global_coords64.to(torch.int32).cpu()
    selected_tile_ids_cpu = selected_tile_ids.to(torch.int32).cpu()
    selected_keys = _coord4_linear_keys(selected_coords_cpu).cpu().numpy()
    selected_tile_np = selected_tile_ids_cpu.cpu().numpy()
    selected_lookup = {
        int(t): set(int(k) for k in selected_keys[selected_tile_np == int(t)])
        for t in np.unique(selected_tile_np)
    }
    ...
    for payload in sorted(payloads, key=lambda x: int(x.tile_id)):
        keep_key_set = selected_lookup.get(int(payload.tile_id), set())
        if not keep_key_set:
            tile_decoder_rows.append({
                "tile_id": int(payload.tile_id),
                "status": "skipped",
                "reason": "no globally selected tokens remain for this tile",
                "selected_source_global_rows": 0,
                "selected_local_c64_tokens": 0,
            })
            continue

        source_keys = _coord4_linear_keys(payload.source_global_coords64.cpu()).cpu().numpy()
        keep_source_mask_np = np.array(
            [int(k) in keep_key_set for k in source_keys],
            dtype=np.bool_,
        )
        if not keep_source_mask_np.any():
            tile_decoder_rows.append({
                "tile_id": int(payload.tile_id),
                "status": "skipped",
                "reason": "no kept source-global rows after tile filtering",
                "selected_source_global_rows": 0,
                "selected_local_c64_tokens": 0,
            })
            continue

        keep_source_mask = torch.from_numpy(keep_source_mask_np).to(torch.bool)
        source_to_local = payload.source_to_local_index.cpu().to(torch.long)
        selected_local_ids = torch.unique(
            source_to_local[keep_source_mask],
            sorted=True,
        )
        if selected_local_ids.numel() == 0:
            tile_decoder_rows.append({
                "tile_id": int(payload.tile_id),
                "status": "skipped",
                "reason": "selected local C64 set is empty",
                "selected_source_global_rows": int(keep_source_mask.sum().item()),
                "selected_local_c64_tokens": 0,
            })
            continue

        local_coords = payload.local_coords64.index_select(
            0, selected_local_ids
        ).to(device=pipeline.device, dtype=torch.int32)
        local_shape_feats = payload.local_shape_denorm_feats.index_select(
            0, selected_local_ids
        ).to(pipeline.device)
        local_texture_feats = payload.local_texture_denorm_feats.index_select(
            0, selected_local_ids
        ).to(pipeline.device)
        ...
        decoded, mesh = _decode_normal_mesh_with_ovoxel(
            ...
            label=f"Tile {int(payload.tile_id):02d} filtered local decode",
        )
        ...
        if official_mesh_template is None:
            ...
            print(
                "[local-decode-merge] canonical O-Voxel lattice: "
                f"origin={canonical_metadata_stats['canonical_origin']} "
                f"voxel_size={canonical_metadata_stats['canonical_voxel_size']:.10f} "
                f"shape={canonical_metadata_stats['canonical_voxel_shape']}"
            )
        ...
        # Decoder output is object space. Convert object -> local q -> global q
        # -> global object before either rendering or global O-Voxel quantization.
        ...
        # Never append geometry without corresponding valid material metadata.
        if quantized_coords3.shape[0] == 0 or compact_faces.shape[0] == 0:
            tile_decoder_rows.append({
                "tile_id": int(payload.tile_id),
                "status": "skipped_after_decode",
                "reason": (
                    "no tile-owned triangles or no tile-owned canonical O-Voxels"
                ),
                "selected_source_global_rows": int(keep_source_mask.sum().item()),
                "selected_local_c64_tokens": int(selected_local_ids.numel()),
                "decoder_vertices": int(mesh.vertices.shape[0]),
                "decoder_faces": int(mesh.faces.shape[0]),
        ...
        tile_decoder_rows.append({
            ...
            "decoder_active_voxels": int(mesh.coords.shape[0]),
            **face_stats,
            "tile_owned_ovoxel_centers": int(voxel_owned.sum().item()),
            "tile_owned_ovoxels_outside_canonical_cube": int(
    ...
    if not success_rows:
        raise RuntimeError("no filtered tile local decode produced mergeable metadata")
    if not vertex_chunks or not face_chunks or not ovoxel_coord_chunks:
        raise RuntimeError("filtered tile local decode produced no mergeable geometry")
    ...
    summary = {
        "attempted_tile_local_decodes": len(payloads),
        "successful_tile_local_decodes": len(success_rows),
        ...
        "merge_route": (
            "decoder local object -> local q -> inverse camera mapping -> global q -> "
            "global object; crop triangles/O-Voxels by projected tile ownership; "
            "weld vertices; resolve O-Voxel collisions by closest tile center; "
            "render with full canonical 1024^3 O-Voxel lattice"
        ),
    }
...
def run(args: argparse.Namespace) -> None:
        ...
        try:
            ...
            tile_local_decode_payloads.append(
                TileLocalDecodePayload(
                    ...
                    source_global_coords64=mapping.source_global_coords64.detach().cpu().to(torch.int32),
                    source_to_local_index=mapping.source_to_local_index.detach().cpu().to(torch.long),
                )
            )
    ...
    local_decode_merge_dir = output_dir / "merged_local_tile_decodes"
    ...
    ) = _decode_filtered_tile_payloads_and_merge_to_global(
        pipeline=pipeline,
        payloads=tile_local_decode_payloads,
        selected_global_coords64=merged_coords64_cpu,
        selected_tile_ids=selected_tile_ids_cpu,
        global_camera=global_camera,
    ...
    local_decode_compare_png = output_dir / "comparison_global_baseline_vs_merged_local_tile_decodes.png"
    ...
    local_decode_summary = {
        ...
        **local_decode_merge_stats,
        "route": (
            "closest-center global token selection -> drop per-tile losing global tokens -> "
            "decode each tile locally -> object-to-q inverse mapping -> tile-owned spatial crop -> full-1024 O-Voxel merge -> render"
        ),
        "comparison_png": str(local_decode_compare_png),
        "covered_support_overlay_png": str(covered_support_overlay_path),
        "tile_local_decode_rows_csv": str(local_decode_merge_dir / "tile_local_decode_records.csv"),
        "tile_local_decode_records": local_decode_tile_rows,
    }
    _write_csv(local_decode_merge_dir / "tile_local_decode_records.csv", local_decode_tile_rows)
    ...
    summary = {
        "format": "pixal3d_tile_slat_return_to_global_object_correct_spatial_merge_v6",
        ...
        "tile_route": [
            ...
            "additional comparison: after token selection, decode each tile locally, convert decoder object coordinates through q, crop by tile ownership, weld, merge on full 1024^3 O-Voxel lattice, and render",
        ],
        "duplicate_policy": {
        ...
        "merged_local_tile_decodes": local_decode_summary,
        "metric_gains_over_baseline": {
            "merged_global_tile_slat": metric_gains,
            "merged_local_tile_decodes": local_decode_metric_gains,
        },
        "comparison_png": str(comparison_path),
