...
# -*- coding: utf-8 -*-
"""
Compare an ordinary global Pixal3D baseline with correspondence-synchronized
overlapping tile flows followed by complete local-decode geometry fusion.
...
Route 1: shared-global-C1024 velocity consensus plus geometry fusion

    global shape1024 latent
    -> decoder learned subdivision to dense global C1024 support
    -> project global C1024 support into every overlapping 1024 tile
    -> continuously recanonicalize global C1024 into each centered tile camera
    -> quantize only after the transform, directly to unique local C64
    -> link local tokens across tiles when they originate from the same exact
       global C1024 coordinate
    -> at every shape and texture Euler-flow step, predict each tile's velocity
       with its own state/image condition
    -> average velocity for linked tokens; unlinked tokens retain their own
       velocity; update every tile once with the synchronized velocity
    -> use every tile's complete synchronized local shape/texture SLAT
...
There is deliberately no local-SLAT-to-global-SLAT return, closest-center
latent candidate selection, or one-global-decode route in this experiment.
...
@dataclass
class PreparedVelocitySyncTile:
    tile_id: int
    box: Tuple[int, int, int, int]
    transform: TileCameraTransform
    ...
    tile_dir: Path
    support_dir: Path
    mapping: TileSupportMapping
    seed: int


@dataclass(frozen=True)
class VelocityCorrespondence:
    tile_ids: Tuple[int, ...]
    node_offsets: torch.Tensor
    incidence_nodes: torch.Tensor
    incidence_groups: torch.Tensor
    incidence_linked_rows: torch.Tensor
    linked_nodes: torch.Tensor
    group_counts: torch.Tensor
    linked_node_group_counts: torch.Tensor
    shared_global_coords1024: torch.Tensor
    stats: Dict[str, Any]


@dataclass(frozen=True)
class VelocitySyncStageResult:
...
def _pack_proj_condition_cpu(
    ...
    *,
    ...
    """Offload one tile condition without retaining a CUDA SparseTensor."""
    packed: Dict[str, Dict[str, torch.Tensor]] = {}
    for branch_name in ("cond", "neg_cond"):
        if branch_name not in condition:
            raise KeyError(f"{label}: missing condition branch {branch_name!r}")
        ...
        if not isinstance(global_value, torch.Tensor):
            raise TypeError(
                f"{label}.{branch_name}.global must be a torch.Tensor"
            )
        if not isinstance(proj_value, SparseTensor):
            raise TypeError(
                f"{label}.{branch_name}.proj must be a SparseTensor"
            )
        if proj_value.feats.shape[0] != expected_coords.shape[0]:
            raise RuntimeError(
                f"{label}.{branch_name}.proj token count is not aligned"
            )
        if not torch.equal(proj_value.coords, expected_coords):
            raise RuntimeError(
                f"{label}.{branch_name}.proj coordinate order changed"
            )
        packed[branch_name] = {
            "global": global_value.detach().to(device="cpu", copy=True),
            "proj": proj_value.feats.detach().to(device="cpu", copy=True),
        }
    return packed
...
def _materialize_proj_condition(
    ...
    *,
    ...
    """Restore one packed tile condition on the current model device."""
    ...
    for branch_name in ("cond", "neg_cond"):
        ...
        if proj_features.shape[0] != coords.shape[0]:
            raise RuntimeError(
                f"{branch_name} projected condition/token count differs"
            )
        output[branch_name] = {
            "global": branch["global"].to(device=device),
            "proj": SparseTensor(feats=proj_features, coords=coords),
        }
    return output


def _sampler_prediction_kwargs(
    params: Mapping[str, Any],
) -> Dict[str, Any]:
    """Keep the exact per-step CFG options while removing loop controls."""
    loop_keys = {
        "steps",
        "rescale_t",
        "verbose",
...
def _global_c1024_linear_keys(coords: torch.Tensor) -> torch.Tensor:
    if coords.ndim != 2 or coords.shape[1] != 4:
        raise ValueError("global C1024 coordinates must have shape [N,4]")
    coords64 = coords.to(device="cpu", dtype=torch.int64)
    if bool((coords64[:, 0] != 0).any().item()):
        raise ValueError("global C1024 correspondence requires batch id zero")
    xyz = coords64[:, 1:4]
    if bool(
        (
            (xyz < 0)
            | (xyz >= GRID_GLOBAL_UPSAMPLED)
        ).any().item()
    ):
        raise ValueError("global C1024 correspondence contains invalid indices")
    return (
        (xyz[:, 0] * GRID_GLOBAL_UPSAMPLED + xyz[:, 1])
        * GRID_GLOBAL_UPSAMPLED
        + xyz[:, 2]
    )


def _build_velocity_correspondence(
    tiles: Sequence[PreparedVelocitySyncTile],
) -> VelocityCorrespondence:
    """Build exact cross-tile incidences through identical global C1024 rows.

    One local C64 token can represent several global C1024 sources after local
    quantization. Each shared source first forms its own velocity consensus.
    A multiply-associated local token receives the mean of those source-level
    consensus proposals, so it still has exactly one Euler update.
    """
    if not tiles:
        raise ValueError("at least one prepared tile is required")
    ...
    for tile in tiles:
        ...
        source_keys = _global_c1024_linear_keys(
            mapping.source_global_coords1024
        )
        local_rows = mapping.source_to_local_index.to(
            device="cpu",
            dtype=torch.int64,
        )
        if source_keys.shape[0] != local_rows.shape[0]:
            raise RuntimeError(
                f"tile {tile.tile_id}: source/local mapping rows differ"
            )
        if torch.unique(source_keys).shape[0] != source_keys.shape[0]:
            raise RuntimeError(
                f"tile {tile.tile_id}: global C1024 sources are not unique"
            )
        token_count = int(mapping.local_coords64.shape[0])
        if local_rows.numel() and (
            int(local_rows.min().item()) < 0
            or int(local_rows.max().item()) >= token_count
        ):
            raise RuntimeError(
                f"tile {tile.tile_id}: source_to_local_index is out of range"
            )
        key_parts.append(source_keys)
        node_parts.append(local_rows + int(node_offsets[-1]))
        source_rows_per_tile.append(int(source_keys.shape[0]))
        node_offsets.append(int(node_offsets[-1]) + token_count)

    all_keys = torch.cat(key_parts).numpy()
    all_nodes = torch.cat(node_parts).numpy()
    order = np.lexsort((all_nodes, all_keys))
    ordered_keys = all_keys[order]
    ordered_nodes = all_nodes[order]
    pair_first = np.ones(ordered_keys.shape[0], dtype=bool)
    pair_first[1:] = (
        (ordered_keys[1:] != ordered_keys[:-1])
        | (ordered_nodes[1:] != ordered_nodes[:-1])
    )
    dedup_keys = ordered_keys[pair_first]
    dedup_nodes = ordered_nodes[pair_first]

    unique_keys, counts = np.unique(dedup_keys, return_counts=True)
    old_groups = np.repeat(
        np.arange(unique_keys.shape[0], dtype=np.int64),
        counts,
    )
    shared_group_mask = counts >= 2
    shared_incidence_mask = shared_group_mask[old_groups]
    old_to_new = np.full(unique_keys.shape[0], -1, dtype=np.int64)
    old_to_new[shared_group_mask] = np.arange(
        int(shared_group_mask.sum()),
        dtype=np.int64,
    )
    incidence_groups_np = old_to_new[
        old_groups[shared_incidence_mask]
    ]
    incidence_nodes_np = dedup_nodes[shared_incidence_mask]
    shared_keys_np = unique_keys[shared_group_mask]

    if incidence_nodes_np.size:
        linked_nodes_np, incidence_linked_rows_np = np.unique(
            incidence_nodes_np,
            return_inverse=True,
        )
        group_counts_np = np.bincount(
            incidence_groups_np,
            minlength=shared_keys_np.shape[0],
        )
        linked_node_group_counts_np = np.bincount(
            incidence_linked_rows_np,
            minlength=linked_nodes_np.shape[0],
        )
    else:
        linked_nodes_np = np.empty((0,), dtype=np.int64)
        incidence_linked_rows_np = np.empty((0,), dtype=np.int64)
        group_counts_np = np.empty((0,), dtype=np.int64)
        linked_node_group_counts_np = np.empty((0,), dtype=np.int64)

    shared_keys = torch.from_numpy(
        np.ascontiguousarray(shared_keys_np)
    ).to(torch.int64)
    shared_z = shared_keys % GRID_GLOBAL_UPSAMPLED
    shared_xy = torch.div(
        shared_keys,
        GRID_GLOBAL_UPSAMPLED,
        rounding_mode="floor",
    )
    shared_y = shared_xy % GRID_GLOBAL_UPSAMPLED
    shared_x = torch.div(
        shared_xy,
        GRID_GLOBAL_UPSAMPLED,
        rounding_mode="floor",
    )
    shared_coords = torch.stack(
        [
            torch.zeros_like(shared_x),
            shared_x,
            shared_y,
            shared_z,
        ],
    ...
    offsets_tensor = torch.tensor(node_offsets, dtype=torch.int64)
    linked_nodes = torch.from_numpy(
        np.ascontiguousarray(linked_nodes_np)
    ).to(torch.int64)
    per_tile: List[Dict[str, Any]] = []
    for index, tile in enumerate(tiles):
        start = node_offsets[index]
        stop = node_offsets[index + 1]
        linked_count = int(
            ((linked_nodes >= start) & (linked_nodes < stop)).sum().item()
        )
        token_count = stop - start
        per_tile.append(
            {
                "tile_id": int(tile.tile_id),
                "local_c64_tokens": int(token_count),
                "global_c1024_source_rows": int(
                    source_rows_per_tile[index]
                ),
                "linked_local_c64_tokens": linked_count,
                "linked_local_c64_fraction": float(
                    linked_count / max(token_count, 1)
                ),
            }
        )

    group_size_histogram = {
        str(int(size)): int((group_counts_np == size).sum())
        for size in np.unique(group_counts_np)
    }
    stats = {
        "relation": (
            "local tokens share an edge iff at least one exact global C1024 "
            "source coordinate is identical across tiles"
        ),
        "multi_source_local_token_rule": (
            "mean the source-level consensus velocity proposals"
        ),
        "tile_count": len(tiles),
        "tile_ids": [int(tile.tile_id) for tile in tiles],
        "total_local_c64_nodes": int(node_offsets[-1]),
        "total_source_membership_rows": int(len(all_keys)),
        "unique_global_c1024_sources": int(unique_keys.shape[0]),
        "shared_global_c1024_sources": int(shared_keys_np.shape[0]),
        "shared_source_incidence_rows": int(incidence_nodes_np.shape[0]),
        "linked_local_c64_nodes": int(linked_nodes_np.shape[0]),
        "unlinked_local_c64_nodes": int(
            node_offsets[-1] - linked_nodes_np.shape[0]
        ),
    ...
    print(
        ...
        f"tiles={len(tiles)} local_nodes={node_offsets[-1]:,} "
        f"unique_global_C1024={unique_keys.shape[0]:,} "
        f"shared_global_C1024={shared_keys_np.shape[0]:,} "
        f"linked_local_nodes={linked_nodes_np.shape[0]:,}"
    )
    return VelocityCorrespondence(
        tile_ids=tuple(int(tile.tile_id) for tile in tiles),
        node_offsets=offsets_tensor,
        incidence_nodes=torch.from_numpy(
            np.ascontiguousarray(incidence_nodes_np)
        ).to(torch.int64),
        incidence_groups=torch.from_numpy(
            np.ascontiguousarray(incidence_groups_np)
        ).to(torch.int64),
        incidence_linked_rows=torch.from_numpy(
            np.ascontiguousarray(incidence_linked_rows_np)
        ).to(torch.int64),
        linked_nodes=linked_nodes,
        group_counts=torch.from_numpy(
            np.ascontiguousarray(group_counts_np)
        ).to(torch.float32),
        linked_node_group_counts=torch.from_numpy(
            np.ascontiguousarray(linked_node_group_counts_np)
        ).to(torch.float32),
        shared_global_coords1024=shared_coords,
        stats=stats,
    )


def _average_corresponding_velocities(
    raw_velocity: torch.Tensor,
    correspondence: VelocityCorrespondence,
) -> Tuple[torch.Tensor, Dict[str, float]]:
    """Apply one exact source-incidence velocity consensus on CPU."""
    if raw_velocity.ndim != 2:
        raise ValueError("raw_velocity must have shape [total_tokens, channels]")
    expected_rows = int(correspondence.node_offsets[-1].item())
    if raw_velocity.shape[0] != expected_rows:
        raise ValueError(
            f"velocity rows {raw_velocity.shape[0]:,} != {expected_rows:,}"
        )
    raw = raw_velocity.to(device="cpu", dtype=torch.float32)
    if correspondence.incidence_nodes.numel() == 0:
        return raw, {
            "linked_node_count": 0.0,
            "mean_abs_velocity_delta": 0.0,
            "max_abs_velocity_delta": 0.0,
            "relative_l2_velocity_delta": 0.0,
            "raw_shared_source_dispersion": 0.0,
        }

    incidence_values = raw.index_select(
        0,
        correspondence.incidence_nodes,
    )
    group_sum = torch.zeros(
        (
            correspondence.group_counts.shape[0],
            raw.shape[1],
        ),
        dtype=torch.float32,
    )
    group_sum.index_add_(
        0,
        correspondence.incidence_groups,
        incidence_values,
    )
    group_mean = group_sum / correspondence.group_counts[:, None]
    incidence_consensus = group_mean.index_select(
        0,
        correspondence.incidence_groups,
    )

    linked_sum = torch.zeros(
        (
            correspondence.linked_nodes.shape[0],
            raw.shape[1],
        ),
        dtype=torch.float32,
    )
    linked_sum.index_add_(
        0,
        correspondence.incidence_linked_rows,
        incidence_consensus,
    )
    linked_mean = (
        linked_sum
        / correspondence.linked_node_group_counts[:, None]
    )
    synchronized = raw.clone()
    synchronized.index_copy_(
        0,
        correspondence.linked_nodes,
        linked_mean,
    )

    raw_linked = raw.index_select(0, correspondence.linked_nodes)
    delta = linked_mean - raw_linked
    dispersion = torch.linalg.vector_norm(
        incidence_values - incidence_consensus,
        dim=1,
    )
    metrics = {
        "linked_node_count": float(
            correspondence.linked_nodes.shape[0]
        ),
        "mean_abs_velocity_delta": float(delta.abs().mean().item()),
        "max_abs_velocity_delta": float(delta.abs().max().item()),
        "relative_l2_velocity_delta": float(
            torch.linalg.vector_norm(delta).item()
            / max(torch.linalg.vector_norm(raw_linked).item(), 1e-12)
        ),
        "raw_shared_source_dispersion": float(
            dispersion.mean().item()
        ),
    }
...
def _extract_tile_proj_conditions_cpu(
    *,
    ...
    """Extract every tile image condition once, then store it on CPU."""
    packed_conditions: List[Dict[str, Dict[str, torch.Tensor]]] = []
    for tile_index, tile in enumerate(tiles):
        ...
        packed_conditions.append(
            _pack_proj_condition_cpu(
                condition,
                expected_coords=coords,
                label=f"{stage} tile {tile.tile_id:02d}",
            )
        )
        print(
            f"[velocity-sync-cond] stage={stage} "
            f"tile={tile.tile_id:02d} "
            f"tokens={coords.shape[0]:,} "
            f"index={tile_index + 1}/{len(tiles)} "
            f"seconds={time.perf_counter() - started:.3f}"
        )
        del condition, coords
        _empty_cuda_cache()
    return tuple(packed_conditions)


@torch.no_grad()
def _run_correspondence_velocity_synced_stage(
    *,
    pipeline: Any,
    sampler: Any,
    model: torch.nn.Module,
    tiles: Sequence[PreparedVelocitySyncTile],
    packed_conditions: Sequence[
    ...
    tile_count = len(tiles)
    if tile_count == 0:
        raise ValueError("velocity-synchronized flow has no tiles")
    if len(packed_conditions) != tile_count or len(seeds) != tile_count:
        raise ValueError("tile conditions/seeds are not aligned")
    if correspondence.tile_ids != tuple(tile.tile_id for tile in tiles):
        raise RuntimeError("velocity correspondence tile order changed")
    if concat_features is not None and len(concat_features) != tile_count:
        raise ValueError("concat_features are not aligned with tiles")
    steps = int(params["steps"])
    rescale_t = float(params.get("rescale_t", 1.0))
    if steps < 1 or noise_channels < 1:
        raise ValueError("steps and noise_channels must be positive")

    times = tuple(
        float(value)
        for value in sampler.timestep_schedule(steps, rescale_t)
    )
    ...
    for features in current_features:
        expected_offsets.append(
            expected_offsets[-1] + int(features.shape[0])
        )
    if not torch.equal(
        correspondence.node_offsets,
        torch.tensor(expected_offsets, dtype=torch.int64),
    ):
        raise RuntimeError("flow states do not match correspondence offsets")

    if pipeline.low_vram:
        model.to(pipeline.device)
    per_tile_seconds = [0.0 for _ in tiles]
    ...
    try:
        for step_index in range(steps):
            ...
            raw_velocity_parts: List[torch.Tensor] = []
            for index, tile in enumerate(tiles):
                call_started = time.perf_counter()
                coords = coords_device[index]
                state = SparseTensor(
                    feats=current_features[index].to(
                        device=pipeline.device
                    ),
                    coords=coords,
                )
                materialized = _materialize_proj_condition(
                    packed_conditions[index],
                    coords=coords,
                    device=pipeline.device,
                )
                call: Dict[str, Any] = {
                    "cond": materialized["cond"],
                    "neg_cond": materialized["neg_cond"],
                    **prediction_kwargs,
                }
                concat_value = None
                if concat_features is not None:
                    concat_value = SparseTensor(
                        feats=concat_features[index].to(
                            device=pipeline.device
                        ),
                        coords=coords,
                    )
                    call["concat_cond"] = concat_value
                ...
                if not isinstance(velocity, SparseTensor):
                    raise TypeError(
                        f"{stage} tile {tile.tile_id}: velocity is not sparse"
                    )
                if (
                    velocity.feats.shape != state.feats.shape
                    or not torch.equal(velocity.coords, coords)
                    or not torch.isfinite(velocity.feats).all()
                ):
                    raise RuntimeError(
                        f"{stage} tile {tile.tile_id}: invalid velocity"
                    )
                raw_velocity_parts.append(
                    velocity.feats.detach().to(
                        device="cpu",
                        dtype=torch.float32,
                        copy=True,
                    )
                )
                per_tile_seconds[index] += (
            ...
            raw_velocity = torch.cat(raw_velocity_parts, dim=0)
            synchronized_velocity, sync_metrics = (
            ...
            for index in range(tile_count):
                ...
                stop = int(
                    correspondence.node_offsets[index + 1].item()
                )
                tile_velocity = synchronized_velocity[start:stop]
                current_features[index] = (
                    current_features[index]
                    - dt
                    * tile_velocity.to(
                        dtype=current_features[index].dtype
                    )
                )
            ...
            print(
                f"[velocity-sync-flow] stage={stage} "
                f"step={step_index:02d}/{steps - 1:02d} "
                f"t={t:.8f}->{t_next:.8f} "
                f"tiles={tile_count} "
                f"linked={int(sync_metrics['linked_node_count']):,} "
                f"mean_abs_dv={sync_metrics['mean_abs_velocity_delta']:.8e} "
                f"rel_l2_dv={sync_metrics['relative_l2_velocity_delta']:.8e} "
                f"seconds={step_seconds:.3f}"
            )
            del raw_velocity_parts, raw_velocity, synchronized_velocity
    finally:
        if pipeline.low_vram:
            model.cpu()
            _empty_cuda_cache()

    return VelocitySyncStageResult(
        normalized_features=tuple(current_features),
        times=times,
        per_tile_prediction_seconds=tuple(
...
def _prepare_global_c1024_tile_mapping(
    *,
    ...
    unique local C64 token used by that tile. The exact C1024 source coordinate
    is retained as the cross-tile correspondence key.
    """
...
def _decode_full_tile_payloads_and_merge_to_global(
    *,
    ...
    """Decode every complete synchronized local SLAT and fuse geometry globally.

    The local decoder receives every local C64 token produced by the tile's
    correspondence-synchronized shape1024 and texture1024 flows.
    """
    if not payloads:
    ...
    *,
    ...
    panels = [
        ...
        _label_panel(
            ...
            (
                "C1024-linked velocity-sync local decode + geometry fusion\n"
                f"shared C1024={coverage_fraction:.4%}\n"
        ...
        _label_panel(
            ...
            "Shared global C1024 velocity correspondences",
        ),
        _label_panel(
        ...
        _label_panel(
            ...
            "Velocity-sync geometry-fusion absolute RGB error",
        ),
    ]
    size = panels[0].width
...
def run(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        "used only for cross-tile support and exact correspondence"
    )
    ...
    records: List[Dict[str, Any]] = []
    prepared_tiles: List[PreparedVelocitySyncTile] = []
    ...
    for tile_id, box in enumerate(boxes):
        ...
        if rows.numel() == 0:
            record = {
                ...
                "prepared_for_velocity_sync": False,
            }
        ...
        record = {
            "status": "prepared_for_velocity_sync",
            "tile_id": int(tile_id),
            "box": list(box),
            "selected_global_c1024_rows": int(rows.numel()),
            "kept_source_global_c1024_rows": int(
            ...
            "prepared_for_velocity_sync": True,
            "velocity_sync_deferred": True,
            "complete_local_decode_deferred": True,
            "shape_noise_seed": int(seed_tile + 201),
            "texture_noise_seed": int(seed_tile + 301),
        ...
        _atomic_json(tile_dir / "summary.json", record)
        print(
            f"[tile-prepared] tile={tile_id:02d} "
            f"local_C64={local_tokens:,} "
            f"global_C1024_sources="
            f"{mapping.source_global_coords1024.shape[0]:,}"
        )

    if not prepared_tiles:
        raise RuntimeError("no tile produced a velocity-synchronization support")

    correspondence = _build_velocity_correspondence(prepared_tiles)
    correspondence_dir = output_dir / "velocity_correspondence"
    correspondence_dir.mkdir(parents=True, exist_ok=True)
    correspondence_pt = correspondence_dir / "global_c1024_to_local_c64.pt"
    torch.save(
        {
            "tile_ids": correspondence.tile_ids,
            "node_offsets": correspondence.node_offsets,
            "incidence_nodes": correspondence.incidence_nodes,
            "incidence_groups": correspondence.incidence_groups,
            "incidence_linked_rows": correspondence.incidence_linked_rows,
            "linked_nodes": correspondence.linked_nodes,
            "group_counts": correspondence.group_counts,
            "linked_node_group_counts": (
                correspondence.linked_node_group_counts
            ),
            "shared_global_coords1024": (
                correspondence.shared_global_coords1024
            ),
    ...
    _atomic_json(
        correspondence_dir / "summary.json",
        correspondence.stats,
    )

    shared_global_c1024 = correspondence.shared_global_coords1024
    covered_support_overlay_path = (
        correspondence_dir / "shared_global_c1024_correspondence.png"
    )
    if shared_global_c1024.shape[0] > 0:
        shared_q_global = _endpoint_indices_to_q(
            shared_global_c1024[:, 1:4],
            GRID_GLOBAL_UPSAMPLED,
        ).to(pipeline.device)
        _, _, shared_uv_4096, _, shared_finite = (
            _project_global_q_to_1024_and_4096(
                shared_q_global,
                global_camera=global_camera,
            )
        ...
        _draw_uv_points(
            image_4096,
            shared_uv_4096[shared_finite],
            shared_q_global[:, 2][shared_finite],
            covered_support_overlay_path,
            "global C1024 sources shared by at least two local tile flows",
        )
        del shared_q_global, shared_uv_4096, shared_finite
    else:
        image_4096.save(covered_support_overlay_path)

    shape_conditions = _extract_tile_proj_conditions_cpu(
        pipeline=pipeline,
    ...
    shape_sync = _run_correspondence_velocity_synced_stage(
        pipeline=pipeline,
        sampler=pipeline.shape_slat_sampler,
        model=shape_model,
        tiles=prepared_tiles,
        packed_conditions=shape_conditions,
    ...
    shape_std, shape_mean = _normalization(
        pipeline.shape_slat_normalization,
        torch.device("cpu"),
        shape_sync.normalized_features[0].dtype,
    )
    shape_denorm_features = tuple(
        features * shape_std + shape_mean
        for features in shape_sync.normalized_features
    )

    texture_conditions = _extract_tile_proj_conditions_cpu(
        pipeline=pipeline,
        tiles=prepared_tiles,
    ...
    texture_model = pipeline.models["tex_slat_flow_model_1024"]
    texture_channels = int(texture_model.in_channels) - int(
        shape_sync.normalized_features[0].shape[1]
    )
    if texture_channels <= 0:
        raise RuntimeError(
            f"invalid synchronized texture noise channels {texture_channels}"
        )
    texture_sync = _run_correspondence_velocity_synced_stage(
        pipeline=pipeline,
        sampler=pipeline.tex_slat_sampler,
        model=texture_model,
        tiles=prepared_tiles,
        packed_conditions=texture_conditions,
        correspondence=correspondence,
        params=params["texture"],
        seeds=[tile.seed + 301 for tile in prepared_tiles],
    ...
    texture_std, texture_mean = _normalization(
        pipeline.tex_slat_normalization,
        torch.device("cpu"),
        texture_sync.normalized_features[0].dtype,
    )
    texture_denorm_features = tuple(
        features * texture_std + texture_mean
        for features in texture_sync.normalized_features
    )
    ...
    velocity_sync_summary = {
        ...
        "correspondence_pt": str(correspondence_pt),
        "shared_support_overlay_png": str(covered_support_overlay_path),
    }
    _atomic_json(
        correspondence_dir / "velocity_sync_flow.json",
        velocity_sync_summary,
    )
    ...
    record_by_tile = {
        int(row["tile_id"]): row
        for row in records
        if row.get("status") == "prepared_for_velocity_sync"
    }
    correspondence_per_tile = {
        int(row["tile_id"]): row
        for row in correspondence.stats["per_tile"]
    }
    for index, tile in enumerate(prepared_tiles):
        coords_cpu = tile.mapping.local_coords64.detach().to(
            device="cpu",
            dtype=torch.int32,
        )
        shape_norm_cpu = shape_sync.normalized_features[index]
        shape_denorm_cpu = shape_denorm_features[index]
        texture_norm_cpu = texture_sync.normalized_features[index]
        texture_denorm_cpu = texture_denorm_features[index]
        local_slat_path = (
            tile.tile_dir / "velocity_synced_local_shape_texture_slat.pt"
        )
        torch.save(
            {
                "tile_id": int(tile.tile_id),
                "tile_camera": asdict(tile.transform),
                "shape_noise_seed": int(tile.seed + 201),
                "texture_noise_seed": int(tile.seed + 301),
                "local_coords64": coords_cpu,
                "local_shape_norm_feats": shape_norm_cpu,
                "local_shape_denorm_feats": shape_denorm_cpu,
                "local_texture_norm_feats": texture_norm_cpu,
                "local_texture_denorm_feats": texture_denorm_cpu,
                "source_global_coords1024": (
                    tile.mapping.source_global_coords1024.detach().cpu()
                ),
                "source_to_local_index": (
                    tile.mapping.source_to_local_index.detach().cpu()
                ),
                "velocity_sync": correspondence_per_tile[tile.tile_id],
            },
            local_slat_path,
        )
        ...
        record = record_by_tile[tile.tile_id]
        record.update(
            {
                "status": "success",
                "shape1024_prediction_seconds": float(
                    shape_sync.per_tile_prediction_seconds[index]
                ),
                "texture1024_prediction_seconds": float(
                    texture_sync.per_tile_prediction_seconds[index]
                ),
                "velocity_sync": correspondence_per_tile[tile.tile_id],
                "local_slat_pt": str(local_slat_path),
            }
        )
        _atomic_json(tile.tile_dir / "summary.json", record)
        print(
            f"[tile-velocity-synced] tile={tile.tile_id:02d} "
            f"local_C64={coords_cpu.shape[0]:,} "
            f"linked={correspondence_per_tile[tile.tile_id]['linked_local_c64_tokens']:,}"
        )

    successful_records = [
        row for row in records if row.get("status") == "success"
    ]
    if not tile_local_decode_payloads:
        raise RuntimeError("velocity synchronization produced no local SLAT")

    baseline_psnr = baseline_metric.get("psnr_db")
    baseline_ssim = baseline_metric.get("ssim")
    baseline_lpips = baseline_metric.get("lpips")
    coverage_fraction = float(
        correspondence.stats["shared_global_c1024_sources"]
        / max(correspondence.stats["unique_global_c1024_sources"], 1)
    )

    local_decode_merge_dir = (
        output_dir / "velocity_synced_complete_local_decode_geometry_fusion"
    )
    ...
    local_decode_pre_render = {
        **local_decode_merge_stats,
        "velocity_sync": velocity_sync_summary,
    ...
    local_decode_compare_png = (
        output_dir
        / "comparison_global_baseline_vs_velocity_synced_local_decode_geometry_fusion.png"
    )
    ...
    local_decode_summary = {
        ...
        **local_decode_merge_stats,
        "route": (
            "per-step exact-global-C1024 velocity consensus for shape and texture "
            "flows -> complete synchronized local SLAT decode -> object/q inverse "
            "camera mapping -> nearest-center spatial ownership -> global O-Voxel "
            "requantization and global-camera render"
        ),
        "velocity_sync": velocity_sync_summary,
    ...
    summary = {
        "format": (
            "pixal3d_global_c1024_correspondence_velocity_sync_"
            "local_decode_geometry_fusion_v1"
        ),
        ...
        "tile_route": [
            ...
            "link cross-tile local tokens by their exact source global C1024 coordinate",
            "at every shape Euler step average linked predicted velocities and update once",
            "at every texture Euler step average linked predicted velocities and update once",
            "retain each unlinked token's own predicted velocity",
            "save and completely decode each synchronized local shape/texture SLAT",
            "merge all local decoder geometry and O-Voxel outputs globally",
        ],
        "velocity_sync_policy": {
            ...
            "multi_source_local_token_policy": (
                "mean source-level consensus proposals, then update the local token once"
            ),
            "latent_feature_fusion_after_flow": False,
            "global_latent_decode": False,
        },
        "local_decode_policy": {
        ...
        "correspondence": {
            **correspondence.stats,
            "shared_global_c1024_fraction": coverage_fraction,
            "mapping_pt": str(correspondence_pt),
            "shared_support_overlay_png": str(
                covered_support_overlay_path
            ),
        },
        "global_baseline_1024": baseline_summary,
        "velocity_sync_flow": velocity_sync_summary,
        "velocity_synced_local_decode_geometry_fusion": local_decode_summary,
        "metric_gains_over_baseline": {
            "velocity_synced_local_decode_geometry_fusion": (
                local_decode_metric_gains
            ),
        },
        "comparison_png": str(local_decode_compare_png),
