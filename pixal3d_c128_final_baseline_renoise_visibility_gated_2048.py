#!/usr/bin/env python3
"""Re-noise ONE final baseline endpoint at t_n, then visibility-gated C64 flow."""
...
FORMAT='c128_final_baseline_renoise_visibility_gated_v1'
...
def make_render_grid(records, path, view, metric):
    ...
    for i, row in enumerate(records):
        ...
        draw.text((x+12,y+65), f"Final baseline + noise t={row['start_t']:.4f} | Flow {12-row['n']} steps",font=small,fill=(255,204,102))
        m=row[metric]
...
def main():
    ...
    final_endpoint=endpoints[-1].to(device)
    fixed_noise=noise.to(device)
    final_endpoint_hash=base.tensor_hash(endpoints[-1])
    noise_hash=base.tensor_hash(noise)
    manifest.update(final_endpoint_path=str(SHARED/'encoded_c128_endpoints/step_12.pt'),
                    final_endpoint_sha256=final_endpoint_hash, noise_sha256=noise_hash,
                    sigma_min=float(sampler.sigma_min), render_resolution=1024, decode_resolution=2048)
    base.atomic_json(OUT/'run_manifest.json',manifest)
    base.atomic_save(OUT/'fixed_noise.pt', {'coords':coords,'epsilon':noise,'sha256':noise_hash})
    # All variants use E_final; never use E_n or integrate a guided prefix.
    results=[]
    for n in range(1,13):
        ...
        if path.is_file():
            ...
            assert saved['record']['final_endpoint_sha256']==final_endpoint_hash
            assert saved['record']['noise_sha256']==noise_hash
            results.append(saved['record']);continue
        start_t=float(schedule[n])
        noise_weight=float(sampler.sigma_min)+(1-float(sampler.sigma_min))*start_t
        state=((1-start_t)*final_endpoint+noise_weight*fixed_noise).cpu()
        assert torch.isfinite(state).all()
        base.atomic_save(OUT/f'noised_states/n_{n:02d}.pt', {
            'coords':coords,'normalized_features':state,'t':start_t,
            'clean_weight':1-start_t,'noise_weight':noise_weight,
            'final_endpoint_sha256':final_endpoint_hash,'noise_sha256':noise_hash})
        trace=[]
        print(f'[variant {n:02d}] E_final + noise at t={start_t:.6f}; gated suffix={12-n}',flush=True)
        ...
        record={'n':n,'skipped_prefix_steps':n,'start_t':start_t,'clean_weight':1-start_t,'noise_weight':noise_weight,'final_endpoint_sha256':final_endpoint_hash,'noise_sha256':noise_hash,'suffix_steps':12-n,'suffix_label':'vis-gated','shape_c128_fixed':True,'steps':trace,'path':str(path),'state_sha256':base.tensor_hash(state)}
    ...
    gt=np.asarray(Image.open(BASELINE/'canonical_1024.png').convert('RGB').resize((1024,1024),Image.Resampling.LANCZOS),np.float32)/255
    mask=np.asarray(Image.open(BASELINE/'raw_ovoxel_render/alpha.png').convert('L').resize((1024,1024),Image.Resampling.NEAREST))>127
    back_gt=np.asarray(Image.open(OUT/'baseline1024_reference/back/render.png').convert('RGB'),np.float32)/255
    back_mask=np.asarray(Image.open(OUT/'baseline1024_reference/back/mask.png').convert('L'))>127
    for r in evaluated:
        pred=np.asarray(Image.open(r['front_render']).convert('RGB'),np.float32)/255
        r['front_vs_gt']=base.compare_metrics(gt,pred,mask)
        back=np.asarray(Image.open(r['back_render']).convert('RGB'),np.float32)/255
        r['back_vs_baseline']=base.compare_metrics(back_gt,back,back_mask)
    ...
    summary={**manifest,'status':'complete','results':evaluated,'baseline_front_vs_gt':base.compare_metrics(gt,baseline_pred,mask),'metrics_definition':{'reference':str(BASELINE/'canonical_1024.png'),'foreground_mask':str(BASELINE/'raw_ovoxel_render/alpha.png'),'mask_rule':'fixed baseline alpha > 127; same pixels for all variants','psnr':'RGB MSE over foreground pixels; data_range=1; saved 8-bit RGB PNG','ssim':'skimage 7x7 uniform-window channelwise SSIM map, averaged over fixed foreground pixels','render':'native 2048 decoded geometry/material; 1024x1024 front/back studio PBR; original canonical 1024 reference and original fixed alpha mask'},'schedule':schedule}
    ...
    import render_c128_final_baseline_renoise_errors as error_plots
    error_plots.main()
