...
class ClassifierFreeGuidanceSamplerMixin:
    ...
    def _inference_model_cfg_pair(
        ...
        **kwargs,
        ...
        """Return raw conditional/unconditional predictions before CFG.

        Calling ``super()`` from this mixin intentionally bypasses both the
        guidance-interval wrapper and this class's CFG merge while retaining
        the sampler's normal model/timestep conversion.
        """
        pred_pos = super()._inference_model(model, x_t, t, cond, **kwargs)
        pred_neg = super()._inference_model(model, x_t, t, neg_cond, **kwargs)
        return pred_pos, pred_neg

    def _rescale_cfg_prediction(
        self,
        x_t,
        t,
        pred_pos,
        ...
        """Apply Pixal3D's original x0-space guidance rescale."""
        if guidance_rescale <= 0:
            return pred_cfg
        x_0_pos = self._pred_to_xstart(x_t, t, pred_pos)
        x_0_cfg = self._pred_to_xstart(x_t, t, pred_cfg)
        std_pos = x_0_pos.std(
            dim=list(range(1, x_0_pos.ndim)), keepdim=True
        )
        std_cfg = x_0_cfg.std(
            dim=list(range(1, x_0_cfg.ndim)), keepdim=True
        )
        ...
        return self._xstart_to_pred(x_t, t, x_0)
    ...
    def _combine_cfg_predictions(
        ...
        """Combine an already-computed prediction pair like original CFG."""
        if guidance_strength == 1:
            return pred_pos
        if guidance_strength == 0:
            return pred_neg
        pred = (
            guidance_strength * pred_pos
            + (1 - guidance_strength) * pred_neg
        )
        return self._rescale_cfg_prediction(
            x_t,
            t,
            pred_pos,
            pred,
